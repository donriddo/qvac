...
<Step>
[In your `qvac.config.*`, add the built-in plugins you’ll need in your project](/v0.6.1/sdk/configuration). For example:
...
[In your `qvac.config.*`, add its `<package_name>/plugin` specifier alongside any built-in plugins](/v0.6.1/sdk/configuration). For example:
