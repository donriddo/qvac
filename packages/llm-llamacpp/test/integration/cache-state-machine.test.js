...
safeTest(
  ...
  async (t) => {
    ...
    t.ok(
      ...
      `cache delta (${delta}) approximately equals tracked tokens (${expectedDelta})`
    )
    const threshold = 20
    t.ok(
...
safeTest(
  'Timeout cancellation before first token keeps cache/timing stats at zero (via model.cancel())',
  { timeout: 600_000 },
  async (t) => {
...
safeTest(
  'Timeout cancellation before first token keeps cache/timing stats at zero (via QvacResponse.cancel)',
  { timeout: 600_000 },
  async (t) => {
    const { model, dirPath } = await setupModel(t, { n_predict: '1024', ctx_size: '4096' })
