...
safeTest(
  'remove_thinking_from_context defaults off for Qwen3',
  {
  ...
  async (t) => {
    ...
    t.is(thinkingDiscards, 0, `default run should report 0 discards (got ${thinkingDiscards})`)
  }
)
...
safeTest(
  'remove_thinking_from_context=true opts into compaction for Qwen3',
  {
    skip: isDarwinX64 || isWindowsX64,
    timeout: 600_000
  },
...
safeTest(
  'Qwen3.5 rejects remove_thinking_from_context opt-in',
  {
    skip: isDarwinX64 || isWindowsX64,
    timeout: 900_000
  },
  async (t) => {
    ...
    let caught = null
    try {
      await runCompletionWithStats(inference, messages, {
        generationParams: { remove_thinking_from_context: true }
      })
    } catch (err) {
      caught = err
    ...
    t.ok(
      /recurrent memory|SSM/i.test(caught?.message || ''),
...
safeTest(
  'Qwen3.5 reject does not leak other generation overrides',
  {
    skip: isDarwinX64 || isWindowsX64,
    timeout: 900_000
  },
