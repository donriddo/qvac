...
function toMarkdown(report) {
  ...
  if (report.totalCompletedRuns != null)
    lines.push(`- Completed runs: ${report.totalCompletedRuns}`)
  ...
  for (const model of report.models) {
    ...
    lines.push(
      '| Quantization | Device | Ctx Size | Batch Size | Ubatch Size | Flash Attn | Threads | Cache K | Cache V | Prompt Case | Status | Load Mean | Load Std | Run Mean | Run Std | TTFT Mean | TTFT Std | TPS Mean | TPS Std | Unload Mean | Unload Std | Prompt Tokens | Generated Tokens | Quality Match | Error |'
    )
    lines.push(
      '|---|---|---:|---:|---:|---|---:|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|'
    )
    for (const item of model.cases) {
      ...
      const deviceCell = item.isBaseline
        ? 'default'
        : runtimeConfig.device != null
          ? String(runtimeConfig.device)
          : ''
      const ctxSizeCell = item.isBaseline
        ? 'default'
        : runtimeConfig['ctx-size'] != null
          ? String(runtimeConfig['ctx-size'])
          : ''
      const batchSizeCell = item.isBaseline
        ? 'default'
        : runtimeConfig['batch-size'] != null
          ? String(runtimeConfig['batch-size'])
          : ''
      const ubatchSizeCell = item.isBaseline
        ? 'default'
        : runtimeConfig['ubatch-size'] != null
          ? String(runtimeConfig['ubatch-size'])
          : ''
      const flashAttnCell = item.isBaseline
        ? 'default'
        : runtimeConfig['flash-attn'] != null
          ? String(runtimeConfig['flash-attn'])
          : ''
      const threadsCell = item.isBaseline
        ? 'default'
        : runtimeConfig.threads != null
          ? String(runtimeConfig.threads)
          : ''
      const cacheKCell = item.isBaseline
        ? 'default'
        : runtimeConfig['cache-type-k'] != null
          ? String(runtimeConfig['cache-type-k'])
          : ''
      const cacheVCell = item.isBaseline
        ? 'default'
        : runtimeConfig['cache-type-v'] != null
          ? String(runtimeConfig['cache-type-v'])
          : ''
      const errorCell =
        item.error && item.error.message ? truncateText(item.error.message, 120) : ''
      lines.push(
