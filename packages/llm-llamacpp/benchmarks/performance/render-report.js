  ...
  for (const f of files) {
    try {
      ...
      if (d && typeof d.desktopDevice === 'string' && d.desktopDevice) {
        resolvedDesktop = d.desktopDevice
        break
      }
    } catch {}
  }
...
function rowsFromFile(file, desktopDevice, meta) {
  ...
  if (doc.device && Array.isArray(doc.results)) {
    ...
    for (const r of doc.results) {
      ...
      const crashed =
        (r.status && String(r.status).toLowerCase() === 'crashed') ||
...
function aggregate(rows) {
  ...
  for (const group of byKey.values()) {
    ...
    if (group.some((r) => r.preAggregated)) {
      out.push({ device, config, crashed: true, tokens: null })
      continue
    }
...
function coverageLines(rows, desktopDevice, devices, expectedShards) {
  ...
  for (const d of mobileDevices)
    lines.push(`| ${d} | ${seenByDevice.get(d).size} / ${expected.length} |`)
  lines.push('')

  const missingEverywhere = expected.filter((k) => !seenAll.has(k))
  if (missingEverywhere.length) {
    lines.push(
      `**${missingEverywhere.length} shard(s) produced no data on any device** (likely a dropped batch):`
    )
...
function mermaidSection(rows, desktopDevice, chartsUrl) {
  const held = {
    ...
    rb: CHART_RB,
    size: CHART_SIZE,
    quant: CHART_QUANT_HELD,
    kv: CHART_KV_DEFAULT
  }
  const pts = atConfig(rows, held).filter(
    (r) => r.device !== desktopDevice && !r.crashed && r.tps !== null
  )
  ...
  return [
    ...
    ...mermaidBar(
      `TPS by device (${cfg})`,
      'TPS',
      devices.map(shortDevice),
      devices.map((d) => byDevice.get(d))
    ),
    ''
  ]
}
...
function render(rows, desktopDevice, meta, addonVersionArg, baselineMap, baseline, chartsUrl) {
  ...
  if (comparing && baseline) {
    ...
    const bLine = metaLine(
      baseline.meta,
      bAddon,
      bHasDesktop,
      mobileRepeats(baseline.rows, bDesktop)
    )
    const idParts = []
  ...
  lines.push(
    'Metrics are addon `runtimeStats`: ' +
      ...
      (comparing
        ? ' Δ = current minus baseline (positive = improvement for TPS/ppTPS, negative = improvement for TTFT).'
        : '') +
    ...
    if (comparing) {
      ...
      for (const r of items) {
        ...
        if (r.crashed) {
          ...
          const dTtft =
            b && !b.crashed && r.ttft !== null && b.ttft !== null ? r.ttft - b.ttft : null
          const dTps = b && !b.crashed && r.tps !== null && b.tps !== null ? r.tps - b.tps : null
          const dPp =
            b && !b.crashed && r.ppTps !== null && b.ppTps !== null ? r.ppTps - b.ppTps : null
          const row = hasTokens
...
function chartSeries(rows, desktopDevice, byKey, order, metric, stdKey) {
  const pts = rows.filter(
    (r) =>
      r.device !== desktopDevice && !r.crashed && r[metric] !== null && byKey(r.config) !== null
  )
...
function svgBarChart(title, unit, cats, series, maxOverride) {
  ...
  if (!maxOverride)
    for (const s of series)
      for (const c of s.cells) if (c) max = Math.max(max, c.mean + (c.std || 0))
  ...
  const out = [
    '<svg viewBox="0 0 ' +
      ...
      ' ' +
      H +
      '" width="100%" preserveAspectRatio="xMidYMid meet" font-family="system-ui,Arial" font-size="12">'
  ]
  for (let g = 0; g <= 4; g++) {
    const v = (niceMax / 4) * g
    const yy = y(v)
    out.push(
      `<line x1="${m.l}" y1="${yy.toFixed(1)}" x2="${W - m.r}" y2="${yy.toFixed(1)}" stroke="#e5e7eb"/>`
    )
    out.push(
      `<text x="${m.l - 6}" y="${(yy + 4).toFixed(1)}" text-anchor="end" fill="#6b7280">${fmt(v, v < 10 ? 1 : 0)}</text>`
    )
  }
  ...
  cats.forEach((cat, ci) => {
    ...
    series.forEach((s, si) => {
      ...
      out.push(
        `<rect x="${bx.toFixed(1)}" y="${by.toFixed(1)}" width="${(barW * 0.92).toFixed(1)}" height="${Math.max(0, m.t + ph - by).toFixed(1)}" fill="${s.color}"><title>${s.name} | ${cat}: ${fmt(cell.mean)} ${unit}</title></rect>`
      )
      if (cell.std) {
        const cx = (bx + barW * 0.46).toFixed(1)
        out.push(
          `<line x1="${cx}" y1="${y(cell.mean + cell.std).toFixed(1)}" x2="${cx}" y2="${y(Math.max(0, cell.mean - cell.std)).toFixed(1)}" stroke="#1f2937"/>`
        )
      }
    })
    out.push(
      `<text x="${(gx + groupW * 0.37).toFixed(1)}" y="${m.t + ph + 18}" text-anchor="middle" fill="#111827">${cat}</text>`
    )
  })
  out.push('</svg>')
...
function renderHtml(rows, desktopDevice, meta, addonVersionArg) {
  ...
  const legend = devices
    .map(
      (d, i) =>
        `<span style="display:inline-flex;align-items:center;margin:0 14px 6px 0"><span style="width:12px;height:12px;background:${CHART_COLORS[i % CHART_COLORS.length]};display:inline-block;margin-right:5px;border-radius:2px"></span>${d}</span>`
    )
    .join('')
  ...
  for (const [title, unit, byKey, order, metric, stdKey, extra] of defs) {
    ...
    for (const b of perBackend)
      for (const s of b.series)
        for (const c of s.cells) if (c) smax = Math.max(smax, c.mean + (c.std || 0))
    for (const b of perBackend)
      charts += svgBarChart(`${title} — ${b.backend.toUpperCase()}`, unit, b.cats, b.series, smax)
  }
  const metaBits = [
    addonVersion && `Addon <code>${addonVersion}</code>`,
    meta.promptTokens && `Prompt ${meta.promptTokens} tok`
  ]
    .filter(Boolean)
    .join(' &middot; ')
...
function main() {
  ...
  if (args.compareDir) {
    ...
    } = loadDir(args.compareDir, desktopDevice)
  ...
  if (args.html)
    fs.writeFileSync(args.html, renderHtml(rows, desktopDevice, meta, args.addonVersion))
}
