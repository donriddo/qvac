...
const SAMPLES_PER_TASK =
  intEnv('QVAC_VLM_SAMPLES') ||
  intEnv('QVAC_PERF_RUNS') ||
  PRESET.samplesPerTask ||
  (isMobile ? 2 : 5)
...
async function fetchFromRegistry(source, destPath) {
  ...
  try {
    ;({ QVACRegistryClient } = require(pkg))
  } catch (_) {
  ...
  try {
    await client.downloadModel(source.path, source.source, {
      outputFile: destPath,
      timeout: 5 * 60 * 1000
    })
  } finally {
...
async function ensureBlob(blob) {
  ...
  if (plan.downloadUrl)
    return ensureModel({ modelName: plan.modelName, downloadUrl: plan.downloadUrl })
