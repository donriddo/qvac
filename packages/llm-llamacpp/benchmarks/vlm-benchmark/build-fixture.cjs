...
const TASKS = {
  textvqa: {
    ...
    metric: 'vqa',
    license: 'CC-BY-4.0',
    attribution: 'TextVQA — images from OpenImages (CC-BY-4.0)'
  },
  vizwiz: {
    dataset: 'lmms-lab/VizWiz-VQA',
    config: 'default',
    split: 'val',
    kind: 'bundled',
    q: 'question',
    ...
    metric: 'vqa',
    license: 'CC-BY-4.0',
    attribution: 'VizWiz-VQA (CC-BY-4.0)'
  },
  gqa: {
    dataset: 'lmms-lab/GQA',
    kind: 'gqa',
    instr: { config: 'testdev_balanced_instructions', split: 'testdev' },
    imgs: { config: 'testdev_balanced_images', split: 'testdev' },
    metric: 'vqa',
    license: 'CC-BY-4.0',
    attribution: 'GQA — images from Visual Genome (CC-BY-4.0)'
  },
  docvqa: {
    ...
    metric: 'anls',
    license: 'Apache-2.0',
    attribution: 'DocVQA — UCSF Industry Documents Library (Apache-2.0)'
  },
  ...
  ai2d: {
    ...
    attribution: 'AI2D — AI2 Diagrams (CC-BY-SA-4.0)'
  }
}
...
async function scanRows(ds, config, split, onRow) {
  for (let off = 0; off < SCAN; off += 100) {
    let data
    try {
      data = await getJson(rowsUrl(ds, config, split, off, 100))
    } catch (e) {
      console.warn(`  scan stop @${off}: ${e.message}`)
      break
    }
...
const extOf = (src) => {
  const m = String(src).match(/\.(png|jpe?g|webp|gif)(?:[?#]|$)/i)
...
function pick(candsRaw) {
  ...
  const fracs =
    PER_TASK === 3
      ? [0.1, 0.4, 0.75]
      : Array.from({ length: PER_TASK }, (_, i) => (i + 0.5) / PER_TASK)
  const seen = new Set()
  const out = []
...
async function main() {
  ...
  for (const task of taskNames) {
    const t = TASKS[task]
    if (!ALLOW_LICENSES.has(t.license))
      throw new Error(`${task}: licence ${t.license} not in allowlist`)
    ...
    const cands =
      t.kind === 'gqa'
        ...
        : t.kind === 'ai2d'
          ? await collectAi2d(t)
          : await collectBundled(t)
    console.log(`  ${cands.length} candidates <=${MAX_SIDE}px`)
    if (!cands.length) {
      console.warn(`  !! no candidates for ${task} — skipping`)
      continue
    }
    ...
    for (let i = 0; i < chosen.length; i++) {
      ...
      items.push({
        ...
        task,
        metric: t.metric,
        prompt: c.promptOverride || c.q + VQA_PROMPT_SUFFIX,
        gold: c.gold,
        image,
        width: c.w,
        height: c.h,
  ...
  for (const f of fs.readdirSync(IMAGES_DIR)) {
    if (/^vlmx-.*\.(png|jpe?g|webp|gif)$/i.test(f) && !keep.has(f)) {
      fs.rmSync(path.join(IMAGES_DIR, f))
      console.log(`pruned images/${f}`)
    }
  }
  console.log(
