...
async function ensureModel(modelConfig) {
  ...
  if (fs.existsSync(modelPath)) {
    ...
    console.log(
      'Model found: ' + modelConfig.filename + ' (' + (stats.size / 1024 / 1024).toFixed(1) + 'MB)'
    )
    return modelPath
  }
...
async function benchmarkModel(modelKey, modelConfig, device, batchSize, splitMode, tensorSplit) {
  ...
  } = await loadModel(modelPath, device, batchSize, splitMode, tensorSplit)
  ...
  try {
    for (const multiplier of MULTIPLIERS) {
      ...
      console.log(
        '  Scale: ' +
          scaleLabel +
          (multiplier > 1 ? ' (' + multiplier + ' x base document)' : ' (base document)')
      )
      console.log('='.repeat(60))

      const document =
        multiplier > 1 ? Array(multiplier).fill(baseDocument).join('\n\n') : baseDocument
...
function generateReport(allModelResults, meta) {
  ...
  for (const mr of allModelResults) {
    ...
    for (const sr of mr.scaleResults) {
      subheading(
        sr.scaleLabel + ' Scale — ' + sr.docStats.words + ' words, ~' + sr.docStats.pages + ' pages'
      )
  ...
  for (const mr of allModelResults) {
    ...
    const scale1 = mr.scaleResults.find(function (s) {
      return s.multiplier === 1
    })
    const scale10 = mr.scaleResults.find(function (s) {
      return s.multiplier === 10
    })
  ...
  for (const mr of allModelResults) {
    ...
    const mediumResults = mr.scaleResults.map(function (sr) {
      return sr.results[1]
    })
    const tpsArr = mediumResults
      .map(function (r) {
        return r.avgTps
      })
      .filter(function (t) {
    ...
    const avgTpsAcrossScales =
      tpsArr.length > 0
        ? round(
            tpsArr.reduce(function (a, b) {
              return a + b
            }, 0) / tpsArr.length
          )
        : null
  ...
  for (const f of findings) {
    ...
    lines.push(
      '  - Peak throughput: ' + f.bestTps + ' TPS (' + f.bestTpsChunk + ' @ ' + f.bestScale + ')'
    )
  ...
  if (findings.length >= 2) {
    ...
    if (faster.bestTps > 0 && slower.bestTps > 0) {
      ...
      lines.push(
        '  - ' + faster.label + ' achieves ' + speedup + 'x higher peak TPS than ' + slower.label
      )
    }
  ...
  lines.push(
    '  - Smaller chunks (64w) offer finer retrieval granularity at the cost of more embeddings'
  )
  lines.push(
    '  - For bulk ingestion, prefer large batch sizes with 256w chunks to maximize throughput'
  )
  ...
  if (findings.length >= 2) {
    const gteF = findings.find(function (f) {
      return f.label.indexOf('GTE') !== -1
    })
    const gemmaF = findings.find(function (f) {
      return f.label.indexOf('Gemma') !== -1
    })
    const gteR = allModelResults.find(function (m) {
      return m.modelKey === 'gte'
    })
    const gemmaR = allModelResults.find(function (m) {
      return m.modelKey === 'gemma'
    })

    if (gteF && gemmaF && gteR && gemmaR) {
      lines.push(padRight('Metric', 36) + padRight(gteF.label, 22) + gemmaF.label)
      lines.push('-'.repeat(W))
      lines.push(
        padRight('Embedding Dimension', 36) + padRight(gteF.dim + 'd', 22) + gemmaF.dim + 'd'
      )
      lines.push(
        padRight('Model Load Time', 36) + padRight(gteF.loadMs + 'ms', 22) + gemmaF.loadMs + 'ms'
      )
      lines.push(
        padRight('Peak TPS', 36) + padRight(String(gteF.bestTps), 22) + String(gemmaF.bestTps)
      )
      ...
      for (const sr of gteR.scaleResults) {
        const gemmaSr = gemmaR.scaleResults.find(function (s) {
          return s.multiplier === sr.multiplier
        })
        if (!gemmaSr) continue
      ...
      if (gteF.dim !== gemmaF.dim) {
        lines.push(
          '  GTE produces ' + gteF.dim + 'd embeddings, Gemma produces ' + gemmaF.dim + 'd.'
        )
        if (gteF.dim > gemmaF.dim) {
          lines.push(
            "  GTE's higher dimension may offer richer representations at the cost of more storage."
          )
        } else {
          lines.push(
            "  Gemma's higher dimension may offer richer representations at the cost of more storage."
          )
        }
      }
...
function writeFullReport(allModelResults, splitMode, tensorSplit) {
  ...
  const devices = [
    ...new Set(
      allModelResults.map(function (mr) {
        return mr.device
      })
    )
  ]
