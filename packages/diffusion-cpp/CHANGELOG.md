...
- Remove `if (gen.width == 512 && gen.height == 512)` block from `SdModel.cpp`. On the JS path this block was unreachable (JS-side defaulting fills both axes before the JSON reaches C++). **Direct C++ callers** that previously relied on this block to auto-detect output dimensions from the input image are now affected: `SdModel::process()` uses the `SdGenConfig` 512×512 default when no dimensions are supplied.
