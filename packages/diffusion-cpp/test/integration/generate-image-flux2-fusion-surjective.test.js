...
safeTest(
  'FLUX2-klein fusion (surjective) — face morphing via feature averaging',
  ...
  async (t) => {
    ...
    try {
      ...
      console.log('\n' + '='.repeat(60))
      console.log('FLUX2-KLEIN FUSION (SURJECTIVE) — FACE MORPHING TEST')
      ...
      // ── Generate (surjective fusion — face morphing) ────────────────────────────
      console.log('\n=== Generating image (surjective fusion — face morphing) ===')
      ...
      // Surjective prompt: emphasize blending and feature averaging, not separation
      const response = await model.run({
        prompt:
          'blend the face of @image1 (von Neumann) and @image2 (Shannon) into one person, merge their facial features equally, average their ages, single unified face, portrait, color photograph, neutral expression',
      ...
      const outPath = path.join(modelDir, 'generate-image--flux2-fusion-surjective-seed20.png')
