...
const { ensureModel, detectPlatform, setupJsLogger, isPng } = require('./utils')
...
test(
  'FLUX2-klein img2img — generates 1024×1024 output on GPU without OOM',
  ...
  async (t) => {
    ...
    try {
      ...
      const response = await model.run({
        prompt:
          'same person, color photograph, modern tech CEO, wearing a gray zip up vest, black studio background',
      ...
      t.is(
        ...
        STEPS,
        `Final diffusion tick is step ${STEPS}`
      )
