...
test('Should detect one language for English text', (t) => {
...
test('Should detect multiple languages (topK=2)', (t) => {
  ...
  t.is(results.length, topK, `Should return exactly ${topK} results`)
...
test('Should return "Undetermined" for empty text in detectOne', (t) => {
  const result = detectOne('')
...
test('Should return array with "Undetermined" for empty text in detectMultiple', (t) => {
...
test('Should handle invalid topK values in detectMultiple', (t) => {
  const text = 'Hello world'

  const results1 = detectMultiple(text, 0)
  ...
  const results2 = detectMultiple(text, -5)
...
test('Should return an array of language probabilities in detectMultiple', (t) => {
  const text = 'Hello. Bonjour. Hola.'
  const results = detectMultiple(text, 3)
...
test('Should get language name from ISO3 code', (t) => {
  ...
  t.is(getLangName('spa'), 'Spanish', 'Should return "Spanish" for "spa"')
...
test('Should get ISO2 code from language name', (t) => {
  ...
  t.is(getISO2FromName('SpAnIsH'), 'es', 'Should handle mixed case input')
})
