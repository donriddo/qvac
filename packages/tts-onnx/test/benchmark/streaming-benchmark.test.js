...
/**
 * Streaming Latency Benchmark for ONNX TTS
 *
 ...
 *
 * Uses the same ENV contract as `rtf-benchmark.test.js`, but does not write a
 * separate artifact by default: it attaches streaming results into the same
 * `rtf-benchmark-*.json` file shape under `summary.streaming`, so the
 ...
 *   QVAC_ONNX_TTS_BENCHMARK_USE_GPU      1 | 0 (default 0)
 *   QVAC_ONNX_TTS_BENCHMARK_BACKEND      cpu | coreml | cuda | directml | nnapi | rocm
 *   QVAC_ONNX_TTS_BENCHMARK_DEVICE       device label for reports
 *   QVAC_ONNX_TTS_BENCHMARK_RUNNER       CI runner label for reports
 *   QVAC_ONNX_TTS_BENCHMARK_LABEL        free-form tag appended to artifact filename
 *   QVAC_ONNX_TTS_STREAMING_WARMUP_RUNS  default 1
 *   QVAC_ONNX_TTS_STREAMING_RUNS         default 3 (streaming is slower to accumulate)
 */
...
function getSettings() {
  ...
  return {
    ...
    numWarmup: getEnvInteger('QVAC_ONNX_TTS_STREAMING_WARMUP_RUNS', 1),
    numRuns: getEnvInteger('QVAC_ONNX_TTS_STREAMING_RUNS', isMobile ? 2 : 3),
    correlation: {
...
test('Streaming benchmark: TTFA + inter-chunk latency', { timeout: 1800000 }, async (t) => {
  const settings = getSettings()
  const backend = resolveBackend(platform, settings.useGPU, settings.backendHint)
  const text = getCorpusText(settings.engine)

  console.log('\n' + '='.repeat(70))
  console.log('ONNX TTS STREAMING LATENCY BENCHMARK')
  ...
  try {
    ...
    // `scripts/perf-report/aggregate-onnx-tts-rtf.js` consumes alongside RTF
    ...
    if (isMobile) {
      ...
      const chunkId = `ttsstream-${Date.now()}`
