...
function toMarkdown(report) {
  ...
  for (const model of report.models) {
    ...
    lines.push(
      '| Quantization | Device | Batch Size | Input | No Mmap | Flash Attn | Status | Load ms | Run ms (avg per repeat) | Unload ms | TPS (mean) | Avg CosSim | Error |'
    )
    lines.push('|---|---|---:|---|---|---|---|---:|---:|---:|---:|---:|---|')
    for (const item of model.cases) {
      ...
      const noMmapCell = item.isBaseline ? 'default' : runtimeConfig.noMmap ? 'on' : 'off'
      const flashAttnCell = item.isBaseline ? 'default' : String(runtimeConfig.flashAttn)
