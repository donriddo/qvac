...
test('getRecognizerModelName defaults to latin for unknown language', async (t) => {
  const model = createModel()
  t.is(
    model.getRecognizerModelName(['klingon']),
    'latin',
    'Unknown language should default to latin'
  )
})
...
test('getRecognizerModelName uses first recognized language in mixed list', async (t) => {
  ...
  t.is(
    model.getRecognizerModelName(['bn', 'ar']),
    'bengali',
    'Should use bengali (checked first in mock)'
  )
})
...
test('getRecognizerModelName handles list with unknown + known language', async (t) => {
  const model = createModel()
  t.is(
    model.getRecognizerModelName(['klingon', 'ru']),
    'cyrillic',
    'Should skip unknown and find cyrillic'
  )
})
...
test('getRecognizerModelName handles list of all unknown languages', async (t) => {
  const model = createModel()
  t.is(
    model.getRecognizerModelName(['klingon', 'elvish', 'dothraki']),
    'latin',
    'All unknown should default to latin'
  )
})
