...
let diagnostics
try {
  diagnostics = require('@qvac/diagnostics')
} catch (e) {
  diagnostics = null
}
...
test('_getDiagnosticsJSON status reflects state correctly', (t) => {
  ...
  t.is(
    JSON.parse(ocr._getDiagnosticsJSON()).status,
    'loaded',
    'should be loaded when configLoaded=true'
  )
  ...
  t.is(
    JSON.parse(ocr._getDiagnosticsJSON()).status,
    'destroyed',
    'should be destroyed when destroyed=true'
  )
})
...
test(
  'round-trip: registerAddon with OCR callback, generateReport shows addon',
  { skip: !diagnostics },
  (t) => {
    diagnostics.reset()
    ...
    t.is(
      report.addons[0].version,
      ocr._packageVersion,
      'addon version should match package version'
    )
