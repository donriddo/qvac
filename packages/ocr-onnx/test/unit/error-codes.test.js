...
test('QvacErrorAddonOcr creates error with FAILED_TO_LOAD_WEIGHTS code', async (t) => {
  const err = new QvacErrorAddonOcr({
    code: ERR_CODES.FAILED_TO_LOAD_WEIGHTS,
    adds: 'model file corrupt'
  })
  t.is(err.code, 9001, 'Error code should be 9001')
  t.ok(
    err.message.includes('model file corrupt'),
    'Error message should contain the adds parameter'
  )
  t.ok(
    err.message.includes('Failed to load weights'),
    'Error message should contain the base message'
  )
})
...
test('QvacErrorAddonOcr creates error with INCOMPLETE_BMP_DATA code', async (t) => {
  const err = new QvacErrorAddonOcr({
    code: ERR_CODES.INCOMPLETE_BMP_DATA,
    adds: '/path/to/truncated.bmp'
  })
...
test('QvacErrorAddonOcr creates error with UNSUPPORTED_BMP_HEADER_SIZE code', async (t) => {
  const err = new QvacErrorAddonOcr({
    code: ERR_CODES.UNSUPPORTED_BMP_HEADER_SIZE,
    adds: '/path/to/old.bmp'
  })
...
test('QvacErrorAddonOcr creates error with INVALID_BMP_PIXEL_DATA code', async (t) => {
  const err = new QvacErrorAddonOcr({
    code: ERR_CODES.INVALID_BMP_PIXEL_DATA,
    adds: '/path/to/bad.bmp'
  })
...
test('QvacErrorAddonOcr creates error with IMAGE_DECODE_FAILED code', async (t) => {
  const err = new QvacErrorAddonOcr({
    code: ERR_CODES.IMAGE_DECODE_FAILED,
    adds: '/path/to/corrupt.jpg'
  })
