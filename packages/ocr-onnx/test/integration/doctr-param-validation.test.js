...
test(
  'DocTR load() rejects when pathRecognizer is missing',
...
test(
  'DocTR load() rejects when pathDetector is missing',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const onnxOcr = new ONNXOcr({
      params: {
...
test(
  'DocTR defaults langList to ["en"] when omitted in doctr mode',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const onnxOcr = new ONNXOcr({
      params: {
...
test(
  'DocTR does not filter unsupported languages from langList',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const onnxOcr = new ONNXOcr({
      params: {
    ...
    t.ok(
      onnxOcr.params.langList.includes('klingon'),
      'Should keep "klingon" (doctr skips language filtering)'
    )
...
test(
  'DocTR does not use pathRecognizerPrefix fallback',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const onnxOcr = new ONNXOcr({
      params: {
    ...
    try {
      ...
      t.ok(
        err.message.includes('pathRecognizer'),
        'Error should mention pathRecognizer not pathRecognizerPrefix'
      )
...
test(
  'getModelKey returns different keys for easyocr and doctr modes',
  { timeout: TEST_TIMEOUT },
  async function (t) {
