 ...
 */
test(
  'Large images are resized internally with coordinates in original space',
  ...
  async function (t) {
    ...
    t.comment(
      'Testing internal resize with image: ' +
        imagePath +
        ' (' +
        originalImageWidth +
        'x' +
    ...
    try {
      ...
      await response
        .onUpdate((output) => {
          ...
          t.comment(
            'Max X coordinate: ' + maxX.toFixed(1) + ', Max Y coordinate: ' + maxY.toFixed(1)
          )
          t.ok(
            hasCoordBeyondResizeThreshold,
            'coordinates should be in original image space (some coords > 1200)'
          )
