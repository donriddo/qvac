...
test(
  'load() rejects when both pathRecognizer and pathRecognizerPrefix are missing',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const onnxOcr = new ONNXOcr({
      params: {
    ...
    try {
      ...
      t.ok(
        err.message.includes('pathRecognizer') || err.message.includes('pathRecognizerPrefix'),
...
test(
  'load() filters unsupported languages before reaching C++ layer',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const onnxOcr = new ONNXOcr({
      params: {
...
test(
  'load() constructs recognizer path from prefix before reaching C++ layer',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const onnxOcr = new ONNXOcr({
      params: {
    ...
    t.ok(
      onnxOcr.params.pathRecognizer.includes('recognizer_latin'),
      'Should construct recognizer path with latin model'
    )
