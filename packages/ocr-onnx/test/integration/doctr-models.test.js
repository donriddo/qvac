...
// -------------------------------------------------------------------
test(
  'DocTR CTC - db_mobilenet + crnn_mobilenet on english.bmp',
...
// -------------------------------------------------------------------
test(
  'DocTR attention - db_resnet50 + parseq on english.bmp',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const imagePath = getImagePath('/test/images/english.bmp')
    ...
    t.comment('Detected: ' + JSON.stringify(texts))
    t.comment(
      formatOCRPerformanceMetrics('[attention resnet+parseq]', stats, texts, { skipReport: true })
    )
...
// -------------------------------------------------------------------
test(
  'DocTR cross - db_mobilenet + parseq (attention) on english.bmp',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const imagePath = getImagePath('/test/images/english.bmp')
    ...
    t.comment('Detected: ' + JSON.stringify(texts))
    t.comment(
      formatOCRPerformanceMetrics('[attention mobilenet+parseq]', stats, texts, {
        skipReport: true
      })
    )
...
// -------------------------------------------------------------------
test(
  'DocTR cross - db_resnet50 + crnn_mobilenet (CTC) on english.bmp',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const imagePath = getImagePath('/test/images/english.bmp')
...
// -------------------------------------------------------------------
test(
  'DocTR attention - db_resnet50 + parseq on basic_test.bmp',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    ...
    t.comment('Detected: ' + JSON.stringify(texts))
    t.comment(
      formatOCRPerformanceMetrics('[attention basic_test]', stats, texts, { skipReport: true })
    )
...
// -------------------------------------------------------------------
test(
  'DocTR straightenPages - should not crash and produce valid output',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const imagePath = getImagePath('/test/images/english.bmp')
...
// -------------------------------------------------------------------
test(
  'DocTR recognizerBatchSize - batch=1 vs batch=16 both produce valid output',
  { timeout: TEST_TIMEOUT * 2 },
  async function (t) {
    const imagePath = getImagePath('/test/images/english.bmp')
    t.comment('Testing recognizerBatchSize=1 vs recognizerBatchSize=16')

    const { results: resultsBatch1 } = await runDoctrOCR(
      t,
      {
        pathDetector: DB_MOBILENET,
        pathRecognizer: CRNN_MOBILENET,
    ...
    const { results: resultsBatch16 } = await runDoctrOCR(
      t,
      {
        pathDetector: DB_MOBILENET,
        pathRecognizer: CRNN_MOBILENET,
    ...
    t.is(
      resultsBatch1.length,
      resultsBatch16.length,
      'Both batch sizes should detect same number of regions'
    )

    // Texts should be identical regardless of batch size
    for (let i = 0; i < Math.min(resultsBatch1.length, resultsBatch16.length); i++) {
      t.is(
        resultsBatch1[i].text,
        resultsBatch16[i].text,
        'Text at index ' + i + ' should match across batch sizes'
      )
    }
...
// -------------------------------------------------------------------
test(
  'DocTR default decoding - should use CTC when unspecified',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const imagePath = getImagePath('/test/images/english.bmp')
    ...
    const { results: resultsDefault } = await runDoctrOCR(
      t,
      {
        pathDetector: DB_MOBILENET,
        pathRecognizer: CRNN_MOBILENET
    ...
    const { results: resultsCTC } = await runDoctrOCR(
      t,
      {
        pathDetector: DB_MOBILENET,
        pathRecognizer: CRNN_MOBILENET,
    ...
    t.is(
      resultsDefault.length,
      resultsCTC.length,
      'default and explicit CTC should detect same number of regions'
    )
