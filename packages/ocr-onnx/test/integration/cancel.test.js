...
test(
  'cancel() during inference causes response to settle within timeout',
  ...
  async function (t) {
    ...
    try {
      ...
      try {
        ...
        t.ok(
          err.message.indexOf('did not settle') === -1,
...
test(
  'pipeline is reusable after cancel – second run produces results',
...
test(
  'cancel() propagates to JS layer without crashing the process',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const { onnxOcr, imagePath } = await createAndLoadOcr(t)
    await onnxOcr.load()
...
test('cancel() before run() does not crash', { timeout: TEST_TIMEOUT }, async function (t) {
  ...
  try {
    ...
    await response
      ...
      .onError((err) => {
        t.comment('Unexpected error after idle-cancel: ' + JSON.stringify(err))
      })
      .await()
...
test(
  'cancel() after completed run does not affect subsequent run',
  { timeout: TEST_TIMEOUT * 2 },
  async function (t) {
    const { onnxOcr, imagePath } = await createAndLoadOcr(t)
    await onnxOcr.load()
    ...
    try {
      ...
      await response1
        ...
        .onError((err) => {
          t.fail('First run error: ' + JSON.stringify(err))
        })
        .await()
