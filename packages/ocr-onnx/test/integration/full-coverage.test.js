...
test('getModelKey returns onnx-ocr-fasttext-easyocr by default', function (t) {
  ...
  t.is(
    ONNXOcr.getModelKey({ anything: true }),
    'onnx-ocr-fasttext-easyocr',
    'Defaults to easyocr when pipelineMode not set'
  )
  t.is(
    ONNXOcr.getModelKey({ pipelineMode: 'doctr' }),
    'onnx-ocr-fasttext-doctr',
    'Returns doctr key when pipelineMode is doctr'
  )
})
...
test(
  'loadWeights catch wraps error as FAILED_TO_LOAD_WEIGHTS',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const onnxOcr = await loadOcr()
    const addonRef = onnxOcr.addon
    ...
    binding.loadWeights = function () {
      throw new Error('simulated loadWeights failure')
    }
...
test(
  'OcrFasttextInterface.unload delegates to destroy',
  { timeout: TEST_TIMEOUT },
  async function (t) {
    const onnxOcr = await loadOcr()
    const addonRef = onnxOcr.addon
